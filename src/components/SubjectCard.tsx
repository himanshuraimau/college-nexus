...
const SubjectCard: React.FC<{ subject: Subject; onBack: () => void }> = ({ subject, onBack }) => {
    return (
        <Card className="p-4 sm:p-8 bg-white/80 backdrop-blur-sm shadow-xl rounded-3xl overflow-hidden relative">
            ...
            <div className="relative z-10">
                <div className="flex flex-col sm:flex-row items-center justify-between mb-6 sm:mb-12">
                    ...
                    <h1 className="text-3xl sm:text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-green-600 via-teal-500 to-blue-600 text-center flex-grow">
                        {subject.subject}
                    </h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-12">
                    ...
                    <div>
                        <h3 className="text-xl sm:text-2xl font-semibold mb-4 sm:mb-6 text-green-800 flex items-center">
                        ...
                        <div className="mt-4 sm:mt-6">
                            <h4 className="text-lg sm:text-xl font-semibold mb-2 sm:mb-3 text-green-700 flex items-center">
...
const InfoBadge: React.FC<{ icon: React.ReactNode; label: string; value: number | string }> = ({ icon, label, value }) => (
    <Badge className="px-3 sm:px-6 py-2 sm:py-3 bg-transparent text-green-800 flex items-center space-x-1 sm:space-x-2 text-sm sm:text-lg hover:bg-white">
        {icon}
        <span>{label}:</span>
        <span>{value}</span>
    </Badge>
...
const ResourceSection: React.FC<{ title: string; items: Record<string, string>; icon?: React.ReactNode }> = ({ title, items, icon }) => (
    <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-4 sm:p-6 shadow-md">
        <h3 className="text-xl sm:text-2xl font-semibold mb-4 sm:mb-6 text-green-800 flex items-center">
